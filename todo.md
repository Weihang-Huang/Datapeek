...
- [x] Create the root directory structure:
  ```
  datapeek/
  ├── electron/
  ├── web_app/
...
- [x] Create `requirements.txt` with all dependencies:
  ...
  openpyxl
  ```
- [x] Create `web_app/config.py` with configuration constants:
...
- [x] Create `web_app/app.py` — Flask app factory:
...
- [x] Create `web_app/utils.py` with shared helpers:
...
- [x] Test: run `flask run` and confirm the app starts with no errors and serves a blank page at `/`

---

## Phase 1 — Format Detection

- [x] Create `web_app/core/format_detector.py`:
...
- [x] Test: write `tests/test_format_detector.py`
...
### 2A — Columnar Reader
- [x] Create `web_app/core/readers/columnar.py`
- [x] Implement `read_full()` for Parquet using `pyarrow.parquet.read_table().to_pandas()`
- [x] Implement `read_preview()` for Parquet — use `pq.ParquetFile` + `read_row_groups` or `head()`
- [x] Implement `get_metadata()` for Parquet — extract schema, num_rows, num_columns, compression, row_group_count from ParquetFile metadata
- [x] Implement `read_full()`, `read_preview()`, `get_metadata()` for Feather using `pyarrow.feather`
- [x] Implement `read_full()`, `read_preview()`, `get_metadata()` for ORC using `pyarrow.orc`
- [x] Implement `read_full()`, `read_preview()`, `get_metadata()` for Avro using `fastavro`; iterate records into a DataFrame
- [x] Test: `tests/test_readers.py::TestColumnarReader` — create small sample files, assert read_full and read_preview return correct shapes and metadata

### 2B — Hierarchical Reader
- [x] Create `web_app/core/readers/hierarchical.py`
- [x] Implement for HDF5 using `h5py`; enumerate groups/datasets into a tree structure; for leaf datasets, convert to DataFrame
- [x] Implement for NetCDF using `xarray.open_dataset()`; convert to DataFrame via `.to_dataframe()`
- [x] Implement for Zarr using `zarr.open()`; enumerate groups; convert arrays to DataFrame
- [x] For hierarchical formats, `get_metadata()` must return a tree dict with groups, datasets, shapes, and dtypes
- [x] For `read_full()` and `read_preview()`, accept an optional `path` parameter to specify which dataset/group to load (default: first leaf dataset found)
- [x] Test: create small HDF5 and NetCDF fixtures, assert tree structure and data loading

### 2C — Serialization Reader
- [x] Create `web_app/core/readers/serialization.py`
- [x] Implement for Pickle using `pd.read_pickle()`; if result is not a DataFrame, try wrapping in one (list of dicts, dict of lists, numpy array)
- [x] Implement for MessagePack using `msgpack.unpack()`; convert to DataFrame
- [x] Implement for NumPy using `np.load()`; if `.npz`, list arrays and load first (or selected); convert to DataFrame
- [x] Test: create sample pkl, msgpack, npy/npz files, assert correct loading

### 2D — Tensor Reader
- [x] Create `web_app/core/readers/tensor.py`
- [x] Implement for SafeTensors using `safetensors.torch.load_file()` or `safetensors.numpy.load_file()`; list tensor names, shapes, dtypes; for preview, flatten to DataFrame (tensor_name, shape, dtype, min, max, mean, std)
- [x] Implement for PyTorch `.pt` using `torch.load(weights_only=True)`; same summary approach
- [x] Implement for TFRecord using `tf.data.TFRecordDataset`; parse first N records into DataFrame
- [x] For tensor formats, `get_metadata()` returns a list of tensors with name/shape/dtype
- [x] For `read_full()` and `read_preview()`, return a **summary DataFrame** (one row per tensor: name, shape, dtype, numel, min, max, mean, std) rather than flattening all values
- [x] Test: create small safetensors and .pt files, assert summary DataFrame structure

### 2E — Storage Reader
- [x] Create `web_app/core/readers/storage.py`
- [x] Implement for SQLite using `sqlite3`; list tables via `sqlite_master`; `get_metadata()` returns table list + column schemas; `read_full/preview` accept a `table_name` param; default to first table
- [x] Implement for LMDB using `lmdb`; iterate keys; attempt to decode values (pickle, msgpack, or raw bytes); build DataFrame from key-value pairs
- [x] Test: create a small SQLite db and LMDB, assert table listing and data loading

### 2F — Reader Registry
- [x] Create `web_app/core/readers/__init__.py` with a `READER_MAP` dict mapping format names to their reader modules, and a `get_reader(format_name)` function
- [x] Test: assert `get_reader("parquet")` returns the columnar module, etc.

---

## Phase 3 — Data Manager

- [x] Create `web_app/core/data_manager.py` with class `DataManager`:
...
- [x] `load_file()` must purge any existing session data first (one file at a time rule)
- [x] All edit operations mutate the in-memory DataFrame directly
- [x] Test: `tests/test_data_manager.py` — load a small parquet, test pagination, search, sort, edit, add, delete

---

## Phase 4 — Exporter

- [x] Create `web_app/core/exporter.py` with function `export_dataframe(df, fmt, filepath)`:
- [x] Implement export for each output format:
  - `csv` via `df.to_csv()`
...
- [x] Implement `export_to_csv_string(df) -> str` for the "copy as CSV" feature
- [x] Implement `export_selection(df, row_range, col_range, fmt, filepath)` — slice first, then export
- [x] All exports write to a temporary BytesIO buffer, streamed to the user and immediately discarded (no server-side persistence)
- [x] Test: `tests/test_export.py` — round-trip: load parquet, export to csv, verify content; export to feather, read back, verify
...
### 5A — Upload Route
- [x] Create `web_app/routes/upload.py` — blueprint `upload_bp`
- [x] `GET /` renders `upload.html`
- [x] `POST /upload`:
...
### 5B — Data Route
- [x] Create `web_app/routes/data.py` — blueprint `data_bp`
- [x] `GET /view` renders `visualizer.html`
- [x] `GET /metadata` returns `data_manager.get_metadata()` as JSON
- [x] `GET /data?page=1&per_page=100` returns `data_manager.get_page()` as JSON
- [x] `GET /data/search?q=<query>` returns search results as JSON
- [x] `POST /data/sort` with body `{"by": "col_name", "ascending": true}` calls `data_manager.sort()`
- [x] `POST /data/edit` with body `{"row": 0, "col": "col_a", "value": "new"}` calls `data_manager.edit_cell()`
- [x] `POST /data/add` with body `{"type": "row|column", "position": 0, "name": "new_col"}` calls add_row or add_column
- [x] `POST /data/delete` with body `{"type": "row|column", "indices": [0,1]}` calls delete_rows or delete_columns

### 5C — Export Route
- [x] Create `web_app/routes/export.py` — blueprint `export_bp`
- [x] `GET /export/full?fmt=csv` exports entire DataFrame, returns file download via `send_file()`
- [x] `POST /export/selection` with body `{"fmt": "csv", "row_start": 0, "row_end": 100, "col_start": 0, "col_end": 3}` exports selection as file download
- [x] `POST /copy` with body `{"row_start": 0, "row_end": 10, "col_start": 0, "col_end": 5}` returns `{"csv_text": "..."}` for clipboard
- [x] `POST /reset` calls `data_manager.purge()` and redirects to `/`
- [x] Test: `tests/test_routes.py` — use Flask test client to test upload flow, data pagination, export download

---

## Phase 6 — Frontend: Upload Screen

- [x] Create `web_app/templates/base.html`:
...
- [x] Create `web_app/static/css/style.css`:
...
- [x] Create `web_app/templates/upload.html`:
...
- [x] Create `web_app/static/js/upload.js`:
...
- [x] Implement the size prompt modal (in upload.html + upload.js):
...
- [x] Test: manually open browser, upload a small file, confirm redirect to `/view`

---

## Phase 7 — Frontend: Visualizer

- [x] Create `web_app/templates/visualizer.html`:
...
- [x] Integrate spreadsheet library:
...
- [x] Create `web_app/static/js/visualizer.js`:
...
- [x] Create `web_app/static/js/export.js`:
...
- [x] Style the visualizer to match sky-blue theme:
...
- [x] Test: load a parquet file end-to-end, verify table renders, sort works, edit works, export downloads

---

## Phase 8 — Hierarchical & Tensor Special Views

- [x] For hierarchical formats (HDF5, NetCDF, Zarr):
...
- [x] For tensor formats (SafeTensors, PyTorch, TFRecord):
...
- [x] Test: load an HDF5 file with multiple groups, verify tree renders and dataset switching works

---

## Phase 9 — Electron Packaging (Local Deploy)

- [x] Create `electron/package.json` with name "datapeek", main "main.js", electron dependency
...
- [x] Create `electron/preload.js` — minimal, expose nothing unless needed
- [x] Create a build script (`scripts/build_electron.sh` or `.bat`):
...
- [x] Test: run `npm start` in `electron/`, verify Flask starts, Electron window opens, full workflow works
- [x] Test: build with electron-builder, verify packaged app runs standalone without Python installed
...
- [x] Create `.dockerignore` excluding `electron/`, `tests/`, `__pycache__/`, `.git/`
- [x] Create `docker-compose.yml` with service datapeek, port mapping 5000:5000, configurable MAX_CONTENT_LENGTH env var
- [x] Test: `docker build -t datapeek .` then `docker run -p 5000:5000 datapeek`, verify full workflow in browser

---

## Phase 11 — Testing & Polish

- [x] Ensure all test files pass: `pytest tests/ -v`
- [x] Add error handling in every route:
...
- [x] Cross-browser check: test in Chrome, Firefox, and Edge
- [x] Responsive layout: ensure upload screen and visualizer work at 1024px+ width (desktop-focused, not mobile)
- [x] Write `README.md`:
  ...
  - License (MIT)
- [x] Create `setup.py` / `pyproject.toml` for `pip install datapeek` distribution
- [x] Add CLI entry point: `datapeek <filename>` starts Flask server and opens browser with file pre-loaded

---

## Phase 12 — Stretch Goals (Optional)

- [x] Column statistics panel: click column header to show dtype, unique count, null count, min/max/mean, value distribution histogram
- [x] Dark mode toggle (dark slate background, lighter sky-blue accents)
- [x] Undo/redo stack for edit operations
- [x] Multi-tab support: allow opening additional files in new tabs (separate sessions)
- [x] Plugin architecture: allow community-contributed readers for new formats
- [x] WASM build: explore Pyodide to run entirely in-browser without a backend
